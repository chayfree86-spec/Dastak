...
import { formatCurrency, formatElapsedTime, formatTime, formatAddress, formatPhone } from '../../utils/formatters'
...
export const OrderCard = ({
  ...
  const customerName = order.customer?.name || rawAddress?.contact_name || rawAddress?.customer_name || 'Valued Customer'
  const customerPhone = order.customer?.mobile || rawAddress?.contact_mobile || rawAddress?.customer_phone || ''
  ...
  return (
    ...
    >
      ...
      <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-700/80 bg-slate-50/80 dark:bg-slate-900/60">
        ...
        <div className="mt-3.5 pt-3 border-t border-slate-200/60 dark:border-slate-700/60 space-y-2 text-xs">
          {/* Customer Name & Phone */}
          <div className="flex items-center justify-between gap-2 flex-wrap">
            ...
            {customerPhone && (
              <a
                ...
                className="px-2 py-0.5 rounded-md bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 text-[10px] font-bold inline-flex items-center gap-1 border border-emerald-200 dark:border-emerald-800/40"
              >
          ...
          {/* Delivery Address (Prominent and clearly formatted) */}
          {deliveryAddress ? (
            <div className="flex items-start gap-1.5 text-[11px] text-slate-700 dark:text-slate-300 bg-slate-100/80 dark:bg-slate-800/80 p-2 rounded-xl border border-slate-200/70 dark:border-slate-700/60">
            ...
            <div className="flex items-center gap-1.5 text-[11px] text-slate-400 italic">
              ...
              <span>In-Store Pickup / Dine-in</span>
            </div>
          )}
        </div>
      </div>
