...
import { Download, Shield, Activity, BarChart3, Share2, PlusSquare, X, CheckCircle2, ExternalLink } from 'lucide-react';
import { usePwaInstall } from '../../hooks/usePwaInstall';

export function PwaInstallModal({
  appName = 'Dastak Admin Portal',
  appRole = 'Multi-Vendor Food & Grocery Operations & Admin Console',
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 mb-6 text-center sm:text-left">
          <div className="relative shrink-0">
            ...
            />
            <div className="absolute -bottom-1 -right-1 bg-indigo-500 text-white p-1 rounded-full border-2 border-slate-900 shadow-md">
          ...
          <div className="flex-1 min-w-0">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold border mb-2 uppercase tracking-wide">
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${accentBadge}`}>
                {isInstalled ? 'App Ready' : 'Admin Console'}
              </span>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
          <div className="bg-slate-800/50 border border-slate-700/40 rounded-2xl p-3.5 flex flex-col items-center text-center">
            ...
            <span className="text-xs font-semibold text-white">Live Metrics</span>
            <span className="text-[11px] text-slate-400 mt-0.5">Real-time GMV & orders</span>
          </div>
          ...
          <div className="bg-slate-800/50 border border-slate-700/40 rounded-2xl p-3.5 flex flex-col items-center text-center">
            ...
            <span className="text-xs font-semibold text-white">Role Access</span>
            <span className="text-[11px] text-slate-400 mt-0.5">Secure operations control</span>
          </div>

          <div className="bg-slate-800/50 border border-slate-700/40 rounded-2xl p-3.5 flex flex-col items-center text-center">
            <div className="w-9 h-9 rounded-xl bg-cyan-500/10 text-cyan-400 flex items-center justify-center mb-2">
              <BarChart3 className="w-4 h-4" />
            </div>
            <span className="text-xs font-semibold text-white">Analytics</span>
            <span className="text-[11px] text-slate-400 mt-0.5">Sales & payout reports</span>
          </div>
        </div>

        {isIOS ? (
          <div className="bg-slate-800/80 border border-indigo-500/30 rounded-2xl p-4 mb-6">
            <div className="flex items-center gap-2 text-indigo-400 text-xs font-bold uppercase tracking-wider mb-2">
              <Share2 className="w-4 h-4" />
              <span>How to Install on iPhone / iPad:</span>
            </div>
            <ol className="text-xs text-slate-300 space-y-2 list-decimal list-inside">
              <li>Tap the <strong className="text-white">Share button</strong> <Share2 className="inline w-3.5 h-3.5 text-indigo-400 mx-0.5" /> in Safari.</li>
              <li>Select <strong className="text-white">Add to Home Screen</strong> <PlusSquare className="inline w-3.5 h-3.5 text-indigo-400 mx-0.5" />.</li>
        ...
        <p className="text-center text-[11px] text-slate-500 mt-4">
          Free install • Secure admin workstation • Desktop & Tablet optimized
        </p>
      </div>
    </div>
  );
