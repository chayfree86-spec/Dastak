...
class RealDemoOrdersSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($customersData as $c) {
            $user = User::where('mobile', $c['mobile'])->orWhere('email', $c['email'])->first();
            if (!$user) {
                $user = User::create([
                    'mobile' => $c['mobile'],
                    'name' => $c['name'],
        ...
        $ridersData = [
            ...
            ['name' => 'Rahul Kashyap', 'mobile' => '9876502345', 'email' => 'rahul.rider.partner@dastakdelivery.com'],
            ['name' => 'Deepak Soni', 'mobile' => '9876503456', 'email' => 'deepak.rider@dastakdelivery.com'],
        ];

        $riders = [];
        foreach ($ridersData as $r) {
            $user = User::where('mobile', $r['mobile'])->orWhere('email', $r['email'])->first();
            if (!$user) {
                $user = User::create([
                    'mobile' => $r['mobile'],
                    'name' => $r['name'],
            ...
            if (!$user->hasRole(UserRole::DELIVERY_BOY)) {
