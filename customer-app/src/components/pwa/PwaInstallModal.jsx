      ...
      >
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 mb-6 text-center sm:text-left">
          <div className="relative shrink-0">
            ...
            />
            <div className="absolute -bottom-1 -right-1 bg-emerald-500 text-slate-950 p-1 rounded-full border-2 border-slate-900 shadow-md">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
          <div className="bg-slate-800/50 border border-slate-700/40 rounded-2xl p-3.5 flex flex-col items-center text-center">
            ...
            <span className="text-xs font-semibold text-white">Super Fast</span>
            <span className="text-[11px] text-slate-400 mt-0.5">Quick browsing & checkout</span>
          </div>
          ...
          <div className="bg-slate-800/50 border border-slate-700/40 rounded-2xl p-3.5 flex flex-col items-center text-center">
            ...
            <span className="text-[11px] text-slate-400 mt-0.5">Live order status updates</span>
          </div>

          <div className="bg-slate-800/50 border border-slate-700/40 rounded-2xl p-3.5 flex flex-col items-center text-center">
            <div className="w-9 h-9 rounded-xl bg-teal-500/10 text-teal-400 flex items-center justify-center mb-2">
              <Smartphone className="w-4 h-4" />
            </div>
            <span className="text-xs font-semibold text-white">Live Tracking</span>
            <span className="text-[11px] text-slate-400 mt-0.5">Real-time rider on map</span>
          </div>
        </div>

        {/* iOS Step-by-Step Helper */}
        {isIOS ? (
          <div className="bg-slate-800/80 border border-emerald-500/30 rounded-2xl p-4 mb-6">
            <div className="flex items-center gap-2 text-emerald-400 text-xs font-bold uppercase tracking-wider mb-2">
              <Share2 className="w-4 h-4" />
              <span>How to Install on iPhone / iPad:</span>
            </div>
            <ol className="text-xs text-slate-300 space-y-2 list-decimal list-inside">
              <li>Tap the <strong className="text-white">Share button</strong> <Share2 className="inline w-3.5 h-3.5 text-emerald-400 mx-0.5" /> in Safari browser.</li>
              <li>Scroll down & select <strong className="text-white">Add to Home Screen</strong> <PlusSquare className="inline w-3.5 h-3.5 text-emerald-400 mx-0.5" />.</li>
              <li>Tap <strong className="text-white">Add</strong> on the top right to finish.</li>
            </ol>
          </div>
        ) : null}
