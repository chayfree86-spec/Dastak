...
import { PlusCircle, Edit2, Trash2, FolderPlus, UtensilsCrossed, ImageOff, Filter, Search } from 'lucide-react'
...
const Thumb = ({ src, size = 'w-12 h-12', veg }) => (
  <div className={`${size} rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0`}>
    {src ? (
      <img src={src} alt="" className="w-full h-full object-cover" loading="lazy" />
    ) : (
      <ImageOff className={`w-4 h-4 ${veg ? 'text-emerald-500' : 'text-slate-400'}`} />
    )}
  </div>
)
...
export const MenuManager = ({ restaurantId }) => {
  ...
  const { data: menu, loading, retry } = useApi(
  ...
  const allItems = useMemo(() => {
    const rows = []
    tree.forEach((c) => {
      ;(c.items || []).forEach((i) => rows.push({ ...i, catId: c.id, categoryName: c.category }))
      ;(c.subcategories || []).forEach((s) =>
        (s.items || []).forEach((i) => rows.push({ ...i, catId: s.id, categoryName: `${c.category} › ${s.category}` }))
      )
    })
    return rows
  }, [tree])
  ...
  const categoryFilterOptions = useMemo(
    () => [{ value: 'ALL', label: 'All Categories' }, ...tree.map((c) => ({ value: c.id, label: c.category }))],
    [tree]
  )

  const selectedTop = categoryFilter !== 'ALL' ? tree.find((c) => c.id === categoryFilter) : null

  // Sub-category filter options depend on the chosen category.
  const subFilterOptions = useMemo(() => {
    if (!selectedTop) return [{ value: 'ALL', label: 'All Sub-categories' }]
    return [{ value: 'ALL', label: 'All Sub-categories' }, ...(selectedTop.subcategories || []).map((s) => ({ value: s.id, label: s.category }))]
  }, [selectedTop])

  // Which category ids match the current category + sub-category filter.
  const allowedIds = useMemo(() => {
    if (categoryFilter === 'ALL') return null
    if (subFilter !== 'ALL') return [subFilter]
    return [selectedTop.id, ...(selectedTop.subcategories || []).map((s) => s.id)]
  ...
  const filteredItems = allItems
    ...
    .filter((i) => {
      ...
      return i.name.toLowerCase().includes(q) || (i.short_code && i.short_code.toLowerCase().includes(q))
    })

  // Most specific selected category (sub if chosen, else top) for edit/delete.
  const activeCategory = useMemo(() => {
    if (subFilter !== 'ALL' && selectedTop) {
      const sub = (selectedTop.subcategories || []).find((s) => s.id === subFilter)
  ...
  const onCategoryChange = (val) => { setCategoryFilter(val); setSubFilter('ALL') }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      if (confirm.type === 'item') {
        ...
        setCategoryFilter('ALL'); setSubFilter('ALL')
      }
      setConfirm(null)
      retry()
    } catch (err) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" icon={FolderPlus} onClick={() => setCatModal({ parentId: null })}>
            Add Category
          </Button>
      ...
      {/* Search + Category + Sub-category filters */}
      <div className="p-3 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
        <div className="flex flex-col md:flex-row md:items-end gap-3">
          ...
          <div className="flex-1 relative">
            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Search Item</label>
          ...
          <div className="flex flex-wrap items-end gap-3 shrink-0">
            <div className="flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-300 shrink-0 pb-2.5">
              <Filter className="w-4 h-4 text-slate-400" />
              <span>Filter</span>
            </div>
            <div className="w-48">
              <CustomSelect label="Category" value={categoryFilter} onChange={onCategoryChange} options={categoryFilterOptions} />
            </div>
            <div className="w-48">
              <CustomSelect
                label="Sub-category"
            ...
            {activeCategory && (
              <div className="flex items-center gap-1 pb-1">
                {!activeCategory.isSub && (
                  <Button variant="ghost" size="sm" icon={FolderPlus} onClick={() => setCatModal({ parentId: activeCategory.id })}>
                    Sub-category
                  </Button>
                )}
        ...
        <div className="text-[11px] text-slate-400 mt-2.5 pt-2 border-t border-slate-100 dark:border-slate-700/60">
          ...
          <div key={item.id} className="p-3.5 flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className={`w-3.5 h-3.5 rounded-sm border-2 flex items-center justify-center shrink-0 ${item.is_veg ? 'border-emerald-600' : 'border-rose-600'}`}>
                  ...
                  {item.short_code && (
                    <span className="px-1 py-0.5 rounded bg-slate-100 dark:bg-slate-700/80 text-slate-500 dark:text-slate-300 font-mono text-[9px] font-bold tracking-wider uppercase shrink-0">
                      {item.short_code}
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-2 text-[11px] text-slate-400 mt-0.5">
                  <span className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 font-medium">{item.categoryName}</span>
            ...
            <div className="flex items-center gap-4 shrink-0">
              <div className="text-right w-20 shrink-0">
                <span className="text-xs font-black text-slate-900 dark:text-slate-100">{formatCurrency(item.discount_price || item.price)}</span>
                {item.discount_price && item.discount_price < item.price && (
                  <span className="text-[10px] text-slate-400 line-through block">{formatCurrency(item.price)}</span>
                )}
              </div>
              <span className={`inline-flex items-center justify-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wide uppercase border w-24 shrink-0 select-none ${
                item.is_available
                ...
                <span className={`w-1.5 h-1.5 rounded-full ${item.is_available ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'}`} />
      ...
      {catModal && (
        <CategoryModal
          ...
          parentOptions={tree.map((c) => ({ value: c.id, label: c.category }))}
          onClose={() => setCatModal(null)}
          onSaved={() => { setCatModal(null); retry() }}
        />
      )}
...
const CategoryModal = ({ restaurantId, parentId, category, parentOptions, onClose, onSaved }) => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        await restaurantsApi.createMenuCategory(restaurantId, { ...payload, parent_id: pid })
  ...
  return (
    <Modal isOpen onClose={onClose} title={isEdit ? 'Edit Category' : parentId ? 'Add Sub-Category' : 'Add Category'} maxWidth="max-w-md">
      <form onSubmit={handleSave} className="space-y-4">
        <Input label="Category Name" required placeholder="e.g. Beverages" value={name} onChange={(e) => setName(e.target.value)} />
        {!isEdit && !parentId && (
          <CustomSelect
        ...
        <ImageUpload label="Category Image (optional)" value={image} onChange={setImage} onRemove={() => setImage(null)} helperText="Any image — it auto-fits." />
...
const ItemModal = ({ restaurantId, tree, item, onClose, onSaved }) => {
  ...
  // Resolve the item's current category + sub-category from its category_id.
  ...
  const fetchTransliteration = async (word) => {
    ...
    try {
      const res = await fetch(`https://inputtools.google.com/request?text=${encodeURIComponent(word)}&itc=hi-t-i0-und&num=4&cp=0&cs=1&ie=utf-8&oe=utf-8&app=demopage`)
  ...
  const categoryOptions = tree.map((c) => ({ value: c.id, label: c.category }))
  const currentCat = tree.find((c) => c.id === Number(categoryId) || c.id === categoryId)
  const subOptions = [
    { value: 'NONE', label: 'None (directly under category)' },
    ...((currentCat?.subcategories || []).map((s) => ({ value: s.id, label: s.category }))),
  ]

  const onCategoryChange = (val) => { setCategoryId(val); setSubId('NONE') }

  const handleSave = async (e) => {
    e?.preventDefault()
    if (!name.trim()) { toast.warning('Name required', 'Please enter an item name.'); return }
    const targetCategoryId = subId && subId !== 'NONE' ? Number(subId) : Number(categoryId)
    if (!targetCategoryId) { toast.warning('Category required', 'Please add / select a category first.'); return }
    if (!price) { toast.warning('Price required', 'Please enter a price.'); return }
    setLoading(true)
    try {
      let imageUrl = typeof image === 'string' ? image : null
      if (image && typeof image !== 'string') {
      ...
      if (isEdit) {
        ...
        // Clear fields to keep modal open for adding more items
        setName('')
        setShortCode('')
        setPrice('')
        setDiscount('')
  ...
  return (
    <Modal isOpen onClose={onClose} title={isEdit ? 'Edit Menu Item' : 'Add Menu Item'} subtitle="Item image is shown to customers and delivery partners." maxWidth="max-w-xl">
      <form onSubmit={handleSave} className="space-y-4">
        {lastAdded && (
          <div className="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 text-xs text-emerald-800 dark:text-emerald-300 flex items-center justify-between">
            <div>
              <span className="font-semibold text-emerald-800 dark:text-emerald-300">Last Item Added:</span> {lastAdded.name} ({formatCurrency(lastAdded.price)})
              {lastAdded.shortCode && <span className="ml-1.5 font-mono font-bold bg-emerald-100 dark:bg-emerald-900/50 px-1 py-0.5 rounded text-[10px]">#{lastAdded.shortCode}</span>}
            </div>
            <span className="text-[10px] bg-emerald-600 text-white font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">Success</span>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="sm:col-span-2">
            <div className="flex items-center justify-between h-6 mb-1.5">
              <label htmlFor="item-name-input" className="text-xs font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-1">
            ...
            {lang === 'HI' && suggestions.length > 0 && (
              <div className="mt-1.5 flex flex-wrap items-center gap-1.5 p-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 shadow-xs">
                <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mr-1 select-none">Suggestions:</span>
                ...
                <span className="text-[9px] text-slate-400 dark:text-slate-500 select-none ml-auto hidden sm:inline">(Space to insert first)</span>
              </div>
            )}
          </div>
          <div>
            <div className="flex items-center h-6 mb-1.5">
              <label htmlFor="item-short-code-input" className="text-xs font-semibold text-slate-700 dark:text-slate-200">
                Short Code
              </label>
            </div>
            <Input
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <CustomSelect label="Category" value={categoryId} onChange={onCategoryChange} options={categoryOptions} />
          <CustomSelect label="Sub-category" value={subId} onChange={setSubId} options={subOptions} disabled={(currentCat?.subcategories || []).length === 0} />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <AmountInput label="Price (₹)" value={price} onChange={(e) => setPrice(e.target.value)} />
          <AmountInput label="Discount Price (₹)" value={discount} onChange={(e) => setDiscount(e.target.value)} />
          <Input label="Prep Time (mins)" type="number" value={prepTime} onChange={(e) => setPrepTime(e.target.value)} />
        </div>
        <Input label="Description (optional)" placeholder="Short description" value={description} onChange={(e) => setDescription(e.target.value)} />
        <ImageUpload label="Item Image" value={image} onChange={setImage} onRemove={() => setImage(null)} helperText="Any size/format — auto-fits everywhere." />
        <div className="space-y-1.5 pt-1">
          <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Food Type</label>
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className={`flex items-center justify-center gap-2 p-2.5 rounded-xl border text-xs font-semibold transition-all ${
                isVeg
                  ? 'border-emerald-600 bg-emerald-50/50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-400 ring-2 ring-emerald-600/20'
            ...
            <button
              ...
              className={`flex items-center justify-center gap-2 p-2.5 rounded-xl border text-xs font-semibold transition-all ${
                !isVeg
                  ? 'border-rose-600 bg-rose-50/50 dark:bg-rose-950/20 text-rose-700 dark:text-rose-400 ring-2 ring-rose-600/20'
                  : 'border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400'
              }`}
            >
              <span className="w-3.5 h-3.5 rounded-sm border-2 border-rose-600 flex items-center justify-center shrink-0">
        ...
        <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-700">
          <Button variant="outline" onClick={onClose} disabled={loading}>Cancel / Close</Button>
